\def\emcolor{\fgyellow}
\definebar[code][method=1,dy=-0.4,offset=-0.3,continue=yes,color=darkgray]
\define[1]\code{{\tt\startbar[code]#1\stopbar}}

\input{prelude-context.tex}

\setuppapersize[A4,landscape][A4,landscape]
\setuppagenumbering[alternative=doublesided]
\def\lfoottext{\pagenumber/\lastpagenumber\hfill\mmtitle, \currentdate}
\def\rfoottext{\hfill\pagenumber/\lastpagenumber}
\setupfootertexts[\footfont{\rfoottext}][][][\footfont{\lfoottext}]

\setuplayout
  [width=fit,
   backspace=8mm,
   topspace=8mm,
   bottomspace=4mm,
   footer=8mm,
   margin=7mm]

\setupfootnotes
  [bodyfont=\footsize,
   location=bottom,
   rule=off,
   before={\blank\hrule\blank[small]}]

\definemixedcolumns
  [sectioncolumns]
  [n=3,
   balance=yes,
   distance=24pt,
   separator=rule,
   rulethickness=.5pt,
   rulecolor=gray]

\setuphead
  [section]
  [style={\switchtobodyfont[\sectionsize]},
   before={\blank[2*big]},
   after={\blank[10pt]\startsectioncolumns},
   aftersection={\stopsectioncolumns}]

\setuphead
  [subsection]
  [style={\switchtobodyfont[\subsectionsize]},
   before={},
   after={\nowhitespace},
   align=right,
   margin=0pt,
   sectionstarter=]

\setuphead
  [subsubsection]
  [style={\switchtobodyfont[\subsubsectionsize]\realbf},
   before={},
   numberstyle={\sectionbar},
   textdistance=3pt,
   distance=3pt]

\definemargindata
  [summary]
  [style=\notefont,location=outer,width=58mm,align=flushleft,distance=3.5mm,stack=continue]

\setupitemize
  [distance=-2pt,
   before={\blank[3pt]},
   after={\blank[3pt]}]

\setuptyping
  [before={\switchtobodyfont[\codesize]\blank[6pt]\blank[disable]},
   after={}]

\setupformula[spacebefore=0pt,spaceafter=nowhite]

\definestartstop[SynSnippetKeyword]  [\c!color=darkred,\c!style=boldface]
\definestartstop[SynSnippetOperator] [\c!color=darkyellow]
\definestartstop[SynSnippetBoundary] [\c!color=darkcyan,\c!style=boldface]
\definestartstop[SynSnippet]         [DefaultSnippet]
\definestartstop[SynSnippetName]     [\c!color=,before={\it}]
\definestartstop[SynSnippetType]     [\c!color=darkblue]
\definestartstop[SynSnippetMagic]    [\c!color=darkmagenta]
\definestartstop[SynSnippetQuote]    [\c!color=darkgreen]
\definestartstop[SynSnippetString]   [\c!color=darkgreen]
\definestartstop[SynSnippetComment]  [\c!color=brownish]

\starttext

\switchtobodyfont[\notesize]

\startnarrow[right=0cm,left=0pt][right]

  \startalignment[center]
    \switchtobodyfont[\titlesize]      \mmtitle   \blank[small]
    \switchtobodyfont[\subsectionsize] \mmauthors \blank[big]
  \stopalignment

  \setupcombinedlist[content][list={section}, alternative=c,]
  \setuplist[section][width=12mm]
  \blank[big]

  \startsectioncolumns
    \placecontent
  \stopsectioncolumns

\stopnarrow
\setupwhitespace[6pt]
\setupmixedcolumns[sectioncolumns][balance=no]
